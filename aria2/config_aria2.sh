...
NGINX_CONFIG_FILE="/etc/nginx/sites-enabled/aria2"
...
service="[Unit] \nDescription=Aria2 Service \nAfter=network.target \n\n[Service] \nExecStart=/usr/bin/aria2c --conf-path=${ARIA2_CONFIG_PATH}/aria2.conf \n\n[Install] \nWantedBy=default.target"
...
# 获取监听端口
RPC_PORT=$(cat /etc/aria2/aria2.conf | grep -oP "rpc-listen-port=\d+" | grep -oP "\d+")
BT_PORT=$(cat /etc/aria2/aria2.conf | grep -oP "listen-port=\d+" | grep -oP "\d+")
DHT_PORT=$(cat /etc/aria2/aria2.conf | grep -oP "dht-listen-port=\d+" | grep -oP "\d+")
...
if [ "${flag,,}" = nginx ]; then
  ...
  ### 设置 nginx 代理 配置
  sudo bash -c "curl ${URL}/nginx-config > ${NGINX_CONFIG_FILE}" && sudo ln ${NGINX_CONFIG_FILE} /etc/nginx/sites-enabled/aria2
